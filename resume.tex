...
%
% Private build (adds phone + work-permit line from the git-ignored private.tex;
% see private.example.tex):
...
% Personal data that must not land in the public, auto-published PDF (phone,
% work-permit status) lives in the git-ignored private.tex and is only pulled
% in when \privatebuild is defined on the command line (see header comment).
...
%-------------------------------------------------------------------------------
% LANGUAGES (+ WORK AUTHORIZATION in the private build)
%-------------------------------------------------------------------------------
\ifdefined\workpermit
  \cvsection{Languages \& Work Authorization}
\else
  \cvsection{Languages}
\fi
\begin{cvskills}
  ...
  \ifdefined\workpermit
    \cvskill{Work authorization}{\workpermit}
  \fi
\end{cvskills}
