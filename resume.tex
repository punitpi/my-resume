...
%
% Just the permit name - no "(Austria)" (the address immediately before it
% already says Austria) and no "no sponsorship required" or similar gloss. An
...
\address{Bregenz, Vorarlberg, Austria
  ...
  {\upshape\bfseries\color{awesome} EU Blue Card}}
...
% Location and work authorization are in the header; repeating them here
% wastes a line. The AWS cert is no longer claimed in the summary because it
% lapsed in Dec 2025 - it appears with its expiry under Certifications.
Comfortable owning a system end to end, from Terraform module to production incident.
\end{cvparagraph}
...
%-------------------------------------------------------------------------------
% cventries, not cvhonors: cvhonors puts the date in a narrow fixed-width
% column on the LEFT, which both wraps a date range and reads inconsistently
% next to Experience, Projects and Education - all of which are cventries with
% the date on the right. Same argument order as those: #1 = the credential
% (small caps, line 2), #2 = the issuer (bold, line 1).
\cvsection{Certifications \& Awards}
\begin{cventries}

  % Issued Dec 2022, lapsed Dec 2025. Shown with the expiry rather than
  % dropped: passing the exam is still evidence, and an undated or
  % present-tense claim would be contradicted by a Credly check.
  \cventry
    {AWS Certified Solutions Architect -- Associate \textnormal{\textit{(expired)}}}
    {Amazon Web Services}
    {}
    {2022 -- 2025}
    {}
  ...
  \cventry
    {Spot Award --- critical project delivered ahead of schedule}
    {OneAdvanced}
    {}
    {}
    {}
